export async function onRequestGet({ env }) {
  try {
    ...
    const testimonials = await env.DB.prepare("SELECT id,name,rating,message FROM testimonials WHERE status='approved' ORDER BY sort_order,id DESC LIMIT 24").all();
    return Response.json({ settings, days:days.results||[], message, songs:songs.results||[], testimonials:testimonials.results||[] }, { headers:{'cache-control':'public,max-age=60'} });
